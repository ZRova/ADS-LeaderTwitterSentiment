...
def gettweets(timefrom):
    ...
    query = '(#BorisJohnson OR Boris Johnson OR bojo OR boris -yeltsin) -is:retweet lang:en -has:media OR entity:"Boris Johnson"' ## add start time: timefrom.strftime("%Y-%m-%dT%H:%M:%SZ")
    tweet_fields = 'author_id,created_at,geo,public_metrics,context_annotations,entities' ## lang is a requirement so not necessary. 
